...
namespace ArchiveSearchEngine.Model.Database
{
    public class DocumentTable
    {
        ...
        public bool CheckIsTempExpiring(string expiring_in)
        {
            ...
            {
                return int.Parse(expiring_in) <= 5;
            }
            return false;
        }
        ...
        private bool CheckIsLongExpiring(string expiring_in)
        {
            if (int.TryParse(expiring_in, out _))
            {
                return int.Parse(expiring_in) > 10;
            }
            return false;
        }
            ...
            string by_year, int startCaseNum, int endCaseNum)
        {
            ...
            else if (doc_type == "Дела по личному составу")
            {
                ...
                check_method = (expiring_in) => { return true; };
            }
        ...
        private string NumToStringConverter(string value)
        {
            ...
            try
            {
                if (value.Length < 7)
                {
                    ...
                    {
                        word += values3AfterTen[int.Parse(value[value.Length - 1].ToString())];
                    }
                    else
                    {
                        if (value.Length == 6)
                        {
                            word += values3[int.Parse(value[0].ToString())] + " ";
                        }
                        if (value.Length >= 5)
                        {
                            if (value[value.Length - 5] == '1')
                            {
                                word += values3AfterTen[int.Parse(value[value.Length - 4].ToString())] + " тысяч ";
                            }
                            else
                            {
                                word += values2[int.Parse(value[value.Length - 5].ToString())] + " ";
                                word += values4[int.Parse(value[value.Length - 4].ToString())] + " ";
                            }
                        }
                        if (value.Length == 4)
                        {
                        ...
                        if (value.Length >= 3)
                        {
                            word += values3[int.Parse(value[value.Length - 3].ToString())] + " ";
                        }
                        if (value.Length >= 2)
                        {
                            word += values2[int.Parse(value[value.Length - 2].ToString())] + " ";
                        }
                        if (value.Length >= 1)
                        {
                            word += values1[int.Parse(value[value.Length - 1].ToString())];
                        }
                    }
                }
            }
